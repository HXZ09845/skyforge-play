...
此前自测修复了引擎脚本下载失败时一直停在加载中、小屏加载页裁切、网页工具遮挡技能的问题。当时通过 290 项 Godot 检查（含 8 项 Viewport 输入分发）及 41 项网页检查，共 331 项；浏览器实际模拟资源请求失败，并验证恢复服务后点重试可进入游戏。

本次浮动摇杆更新通过 14 项输入分发、22 项手机布局与流程、41 项网页检查。在桌面 Chromium 中验证了横屏及网页旋转后的左半屏拖动；实际手机手感仍需真机复测。
