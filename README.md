...
新增三类敌机，替换部分原有刷怪名额，不额外堆数量，也不增加手机按键。

| 敌机 | 行为 | 应对与奖励 |
|---|---|---|
| 护盾机（青色圆环） | 为 5 米内普通敌机减伤 45%，跟随友机；护盾不叠加，不保护自身、其他护盾机、运输机或首领 | 优先击毁护盾机，解除周围敌机的保护 |
| 轰炸机（宽翼、橙色弹舱） | 每约 3.8 秒标记玩家当时位置，红框预警 1.25 秒后爆炸，命中造成 16 伤害 | 红框位置固定，及时移出；Q 无法清除地面轰炸区 |
| 运输机（黄色货舱） | 横向穿行，12 秒战斗时间后逃离；暂停冻结倒计时 | 击毁必掉一件火力、一件射速，基础 300 分乘连击倍率；逃离无击杀、分数和掉落，不阻塞波次 |

第一关首波保持原样，第二波引入护盾机与远程护航，第三波出现运输机。第二、三关每波混入护盾机、射击敌机和轰炸机；运输机另出现在第二关第二波、第三关第三波。每类首次出现会显示应对提示。全程仍为 72 个敌机名额和 3 名首领；运输机逃离时，实际击杀数会少于 75。

## 首领破防

每次首领完成一轮弹幕后，核心会短暂变成青色并显示「核心破防」。窗口约 0.9 秒，此时命中首领的伤害提高 65%；错过窗口就要先躲过下一轮攻击。手机和电脑共用同一套提示，不增加额外按键。
...
从第二关起，第 2、3 波的最后一架敌机有 70% 概率升级为精英目标：体型更大、耐久约为普通敌机的 1.8 倍，近战伤害更高，并以金色血条标记。击败精英目标获得 250 分、2 点超载能量，并必定掉落射速道具，适合在风险更高时追求高分；精英仍须击毁才能完成波次，只有运输机可以自行逃离。
...
当前是三关试玩 Demo。暂无关卡选择或永久数值养成。成绩记录和断点存档不跨设备、浏览器同步；清除站点数据或使用无痕模式可能丢失记录。
